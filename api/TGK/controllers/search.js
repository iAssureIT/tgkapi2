  ...
  if(req.body.floor != ""){
      if(req.body.floor.includes("-")){
        ...
        for(var i=minFloor; i<=maxFloor; i++){
          floorSelector.push({"propertyDetails.floor" : String(i)})
        }
      ...
      if(req.body.floor.includes("-1")){
        selector.push({"propertyDetails.floor" : "-1"}) ;
      }
    ...
    if(req.body.floor === "0" ){
      selector.push({"propertyDetails.floor" : "0"})
    }
                            ...
                            for(var i=0; i<iprops.length; i++){
                                for(let j=0; j<searchResults.length; j++){
                                    ...
                                    if(iprops[i].property_id === searchResults[j]._id){
                                      console.log("condition status-------------------------------->",iprops[i].property_id === searchResults[j]._id);
